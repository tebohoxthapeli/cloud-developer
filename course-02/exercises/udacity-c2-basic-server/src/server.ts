...
(async () => {
    ...
    app.get("/cars/:id", (req: Request, res: Response) => {
        ...
        if (foundCar) return res.status(200).json({ car: foundCar });
    ...
    app.post("/cars/add", (req: Request, res: Response) => {
        const { make, id, type, model, cost }: Car = req.body;
        ...
        return res.status(400).json({
            error: "Invalid car details entered. Ensure that the make, type, model, id and cost are included.",
        });
    });
